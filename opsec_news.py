...
def process_and_send_article(article_url: str, article_title: str, sent_urls: set, fallback_text: str = "") -> bool:
    ...
    # LLM Prompt Engineering: Instructed in English, Output in requested Arabic dialect
    prompt = f"""
    You are an expert Cyber Intelligence Analyst. Draft a precise security report based ONLY on the following extracted text.
    
    STRICT INSTRUCTIONS:
    1. Output Language: Egyptian Arabic (لهجة مصرية صحيحة وواضحة وبسيطة وسلسة).
    2. Analyze the full text and extract the top 3 to 5 highly technical or security-related details.
    3. NO REPETITION. Each point must convey entirely new information.
    4. NO HALLUCINATION. Do not invent facts. If the text lacks deep technical details, limit to 2 points.
    
    Original Title: {article_title}
    Extracted Article Content: {article_content}
    
    REQUIRED FORMAT (Copy the Arabic structure exactly):
    <b>📝 ملخص شامل:</b>
    (Write a professional paragraph summarizing the event/article based on the text)
    
    <b>🛡️ أبرز التفاصيل الفنية:</b>
    🔹 (Point 1)
    🔹 (Point 2)
    🔹 (Point 3)
    
    <b>⚠️ ده هيفرق معاك في إيه؟:</b>
    (A quick, direct security advice for the user based on the news. If no direct security impact, write: "الحدث تقني/إخباري ولا يتطلب إجراء أمني مباشر")
    """
